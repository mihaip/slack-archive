...
func newConversationArchive(conversation Conversation, slackClient *slack.Client, account *Account, devMode bool, dateString string) (*ConversationArchive, error) {
	messages := make([]*slack.Message, 0)
	window, err := archiveWindow(account, devMode, dateString)
	if err != nil {
		return nil, err
	}
	archiveStartTime := window.StartTime
	archiveEndTime := window.EndTime
